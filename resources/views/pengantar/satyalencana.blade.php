...
@section('isi')
    <div class="h-12 mt-2 text-right bg-white">
        <a class="tombol bg-success" href="/SuratSat">Surat</a>
        <input type="text"
            id="searchInput"
    ...
    <h1 class="mt-10 font-sans text-2xl font-bold text-center">
        SURAT PENGANTAR USULAN SATYALENCANA KARYA SATYA
    </h1>
    <div class="mx-auto mt-10   w-full flex justify-center">
        ...
        @can('operator')
            <form action="/P_Sat_op" method="POST">
                @csrf
                    <div>
                        ...
                        <?php $count = 1; ?>
                            @foreach ($buku as $item)
                                <tr class="hover">
                                    ...
                                    <td><a class="tombol bg-red-600" href="/HapusSat/{{ $item->nip }}">Hapus</a></td>
                                </tr>
                                @php
                                    $count++;
                                @endphp
                            ...
                            <?php $count = 1; ?>
                                @foreach ($surat as $item)
                                    <tr class="hover">
                                        ...
                                        <td><a class="tombol bg-success" href="/verifikasi_sat/{{ $item->ID }}">Verifikasi</a></td>
                    ...
                    @foreach ($surat as $s)
                        <tr class="hover">
                            ...
                            @if (($s->tolak != null)&&($s->fase==1))
                                <td>
                                    <a class="tombol bg-success" href="/verifikasi_sat/{{ $s->ID }}">Ajukan Lagi</a><br>
                            ...
                            @elseif ($s->fase == 5)
                                ...
                                <td></td>
                            @else
                                <td><a class="tombol bg-success" href="/verifikasi_sat/{{ $s->ID }}">Ajukan</a></td>
                            @endif

                            <td>{{ $s->tolak }}</td>

                            @if($s->fase == 1)
                                <td><a class="tombol bg-error" href="/del_sat/{{ $s->ID }}">Hapus</a></td>
                            @endif
                        </tr>
                        @php
                            $count++;
