...
@section('isi')
    <div class="h-12 mt-2 text-right bg-white">
        <a class="tombol bg-success" href="/SuratKartu">Surat</a>
        <input type="text"
            id="searchInput"
    ...
    <h1 class="mt-10 font-sans text-2xl font-bold text-center">
        SURAT PENGANTAR PENGURUSAN KARTU SUAMI / KARTU ISTRI
    </h1>
    <div class="mx-auto mt-10   w-full flex justify-center">
        <div class="overflow-auto w-[90%] p-10 rounded-xl shadow-xl odd:bg-white even:bg-slate-50 outline-1">
            @can('operator')
                <form action="/P_kartu_op" method="POST">
                    @csrf
                    <div>
                        ...
                        <tbody id="tableBody">
                            ...
                            @foreach ($buku as $item)
                                <tr class="hover">
                                    ...
                                    <td><a class="tombol bg-red-600" href="/HapusKartu/{{ $item->nip }}">Hapus</a></td>
                                </tr>
                                @php
                                    $count++;
                                @endphp
                    ...
                    <tbody id="tableBody">
                        ...
                        @foreach ($surat as $item)
                            <tr class="hover">
                                ...
                                <td><a class="tombol bg-success" href="/verifikasi_kartu/{{ $item->ID }}">Verifikasi</a></td>
                                <td><a class="tombol bg-red-600" href="/tolak_show/{{ $item->ID}}">Tolak</a></td>
                            </tr>
                            @php
                                $count++;
                            @endphp
                    ...
                    @foreach ($surat as $s)
                        <tr class="hover">
                            ...
                            @if (($s->tolak != null)&&($s->fase==1))
                                <td>
                                    <a class="tombol bg-success" href="/verifikasi_kartu/{{ $s->ID }}">Ajukan Lagi</a><br>
                            ...
                            @elseif ($s->fase == 5)
                                ...
                                <td></td>
                            @else
                                <td><a class="tombol bg-success" href="/verifikasi_kartu/{{ $s->ID }}">Ajukan</a></td>
                            @endif

                            <td>{{ $s->tolak }}</td>

                            @if($s->fase == 1)
                                <td><a class="tombol bg-error" href="/del_kartu/{{ $s->ID }}">Hapus</a></td>
                            @endif
                        </tr>
                        @php
                            $count++;
