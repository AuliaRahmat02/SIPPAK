...
@section('isi')
    <div class="h-12 mt-2 text-right bg-white">
        <a class="tombol bg-success" href="/SuratNaik">Surat</a>
        <input type="text"
            id="searchInput"
    ...
    <h1 class="mt-10 font-sans text-2xl font-bold text-center">
        SURAT PENGANTAR PENGURUSAN KENAIKAN PANGKAT
    </h1>
                        ...
                        <?php $count = 1; ?>
                            @foreach ($buku as $item)
                                <tr class="hover">
                                    ...
                                    <td><a class="tombol bg-red-600" href="/Hapussend/{{ $item->nip }}">Hapus</a></td>
                                </tr>
                                @php
                                    $count++;
                                @endphp
                        ...
                        <tbody id="tableBody">
                            <?php $count = 1; ?>
                                @foreach ($surat as $item)
                                    <tr class="hover">
                                        <th> {{ $count }} </th>
                                        <td>{{ $item->nama_surat}}asdfasd</td>
                                        <td>{{ $item->nomor_surat }}</td>
                                        <td><a class="tombol bg-info" href="/preview_surat/{{ $item->ID }}">Lihat surat</a></td>
                                        <td><a class="tombol bg-success" href="/verifikasi_naik/{{ $item->ID }}">Verifikasi</a></td>
                    ...
                    @foreach ($surat as $s)
                        <tr class="hover">
                            ...
                            @if (($s->tolak != null)&&($s->fase==1))
                                <td>
                                    <a class="tombol bg-success" href="/verifikasi_naik/{{ $s->ID }}">Ajukan Lagi</a><br>
                            ...
                            @elseif ($s->fase == 5)
                                ...
                                <td></td>
                            @else
                                <td><a class="tombol bg-success" href="/verifikasi_naik/{{ $s->ID }}">Ajukan</a></td>
                            @endif

                            <td>{{ $s->tolak }}</td>

                            @if($s->fase == 1)
                                <td><a class="tombol bg-error" href="/hapus_surat/{{ $s->ID }}">Hapus</a></td>
                            @endif
                        </tr>
                        @php
                            $count++;
