...
<div class="h-12 mt-2 text-right bg-white">
    <a class="tombol bg-info" href="/duk/export">EXPORT EXCEL</a>
    <input type="text"
        id="searchInput"
                ...
                <tbody id="tableBody">
                    ...
                    @foreach ($buku as $item)
                        <tr class="hover">
                            ...
                            <td>{{ $item->pangkat }} <br>{{ $item->golru_nm }} <br> {{\Carbon\Carbon::parse($item->tmt_pns)->format("d-m-Y")}} </td>
                            <td> {{ $item->jabatan_nm }} </td>
                            <td> {{ $item->sub_opd_nm }} </td>
                        </tr>
                        @php
                            $count++;
                        @endphp
